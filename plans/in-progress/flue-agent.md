...
- **Follow-up (same PR, post-review, shepherded):** review flagged two blocking issues, both fixed before merge — `getVerifyMoveStatus` accepted a client-supplied `instanceId` with no repo scoping (any user with access to *some* repo could poll another repo's verify logs; fixed with a `verify_move_instances` mapping table + migration enforcing `instanceId → repoId`), and an early clone-failure return (or any thrown exception, e.g. malformed `package.json`) skipped `destroySandbox`, which could strand the account's one container slot (`max_instances: 1`); fixed by wrapping the workflow body in try/finally. One non-blocking item deliberately deferred: `runVerify`'s poll loop has no generation counter/`AbortController`, so two overlapping runs (e.g. a double-click) could race on which result "wins" — blast radius is UI flicker, not a wrong commit gate, since verification only ever checks the default branch. Merged as #17.
