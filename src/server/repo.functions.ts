...
import { loadPlanDetail, loadRepoPlans, resolveAccessibleRepo } from './plans.server'
...
function validateRepoInput(data: RepoInput): RepoInput {
  if (!data?.owner || !data?.repo) throw new Error('owner and repo are required')
...
export const getRepoPlans = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ context, data }): Promise<RepoPlans> => {
    const db = getDb()
    const ctx = await resolveAccessibleRepo(db, context.user.id, data.owner, data.repo)
